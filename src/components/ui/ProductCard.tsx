...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <div
      className="group relative flex flex-col rounded-xl sm:rounded-2xl bg-[#13161C] border border-white/5 overflow-hidden transition-[transform,border-color,box-shadow] duration-150 hover:border-[#8C7A5B]/35 active:border-[#8C7A5B]/50 active:scale-[0.97] hover:shadow-2xl hover:shadow-[#8C7A5B]/5 touch-manipulation"
      ...
      <div className="flex flex-col p-2.5 sm:p-3 lg:p-4 flex-1 justify-between gap-1">
        <div>
          ...
          <div className="flex items-center justify-between text-xs text-gray-400 mb-1 sm:mb-1.5 gap-2">
            <span className="uppercase tracking-widest text-[9px] sm:text-[10px] font-semibold text-[#8C7A5B] truncate">
          ...
          <h3
            onClick={handleQuickView}
            className="font-serif text-xs sm:text-sm lg:text-base font-medium text-gray-100 hover:text-[#8C7A5B] active:text-[#8C7A5B] transition-colors cursor-pointer line-clamp-1 mb-0.5 leading-tight"
          >
            {product.name}
          </h3>
        ...
        <div className="flex items-center justify-between pt-2 border-t border-white/5 mt-auto gap-2">
            ...
            {((product as any).colors as any[])?.map((c: any) => (
              <button
                ...
                className={`w-5 h-5 sm:w-3.5 sm:h-3.5 rounded-full border-2 sm:border transition-all touch-manipulation ${
                  selectedColor === c.name ? 'ring-2 ring-[#8C7A5B] ring-offset-1 ring-offset-[#13161C] border-white' : 'border-white/20 hover:scale-110 active:scale-95'
                }`}
          ...
          <div className="flex items-center gap-1 sm:gap-2 min-w-0">
            <span className="text-xs sm:text-sm font-semibold text-[#EFECE6] tracking-tight truncate">
              {formatPrice(Number(product.price))}
            </span>
          </div>
        </div>
