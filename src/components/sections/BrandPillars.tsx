  ...
  const pillars = [
    {
      ...
      description: 'Worldwide insured courier. Ilorin next-day, rest of Nigeria 2–3 days, signature required.',
    },
    {
      icon: ShieldCheck,
  ...
  return (
    <section id="heritage" className="bg-[var(--color-surface-alt)] border-y border-[var(--color-border)] relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10 lg:py-12">
        <div className="flex sm:grid sm:grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-5 lg:gap-6 overflow-x-auto sm:overflow-visible pb-3 sm:pb-0 snap-x snap-mandatory scrollbar-none -mx-4 sm:mx-0 px-4 sm:px-0">
          {pillars.map((item, idx) => {
            const Icon = item.icon;
            return (
              <div
                key={idx}
                className="snap-start shrink-0 w-[235px] sm:w-auto flex flex-col items-center text-center p-3.5 sm:p-5 rounded-2xl bg-white border border-[var(--color-border)] hover:border-[var(--color-accent-gold)]/40 hover:shadow-[var(--shadow-elev-1)] active:scale-[0.97] transition-all group touch-manipulation"
              >
