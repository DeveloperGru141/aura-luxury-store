      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 lg:gap-10 items-center">
          <div className="lg:col-span-4 flex flex-col items-start text-left w-full min-w-0">
            <p className="text-xs font-medium tracking-wide text-[#D4AF37] mb-3 sm:mb-4 reveal-subhead">Handcrafted in Ilorin — leather, silk & Swiss-sourced timepieces</p>
            ...
            <h1 className="font-serif text-[clamp(28px,7vw,36px)] sm:text-5xl lg:text-[42px] xl:text-5xl font-light text-white tracking-tight leading-[1.12] sm:leading-[1.14] mb-3 sm:mb-4 w-full reveal-headline">
              ...
              <span className="text-base font-normal text-gray-400">Timepieces, sourced from Switzerland.</span>
            </h1>
          ...
          <div className="lg:col-span-8 relative w-full min-w-0">
            ...
            >
              {heroSlides.length === 0 ? (
                ...
                <>
                  {/* Mobile: side-by-side scroll-snap carousel */}
                  <div className="flex gap-3 overflow-x-auto snap-x snap-mandatory md:hidden -mx-4 px-4 pb-2">
                    {heroSlides.slice(0, 4).map((slide, idx) => (
                      <div
                        key={slide.id}
                        className="relative shrink-0 w-[calc(50%-6px)] snap-start rounded-2xl overflow-hidden"
                      >
                        <Image
                          src={slide.image}
                          alt={slide.title}
                        ...
                        <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent p-3 flex items-end justify-between gap-2">
                          <div className="min-w-0">
                            <div className="flex items-center gap-1 mb-0.5">
                              ...
                              <span className="text-amber-400 text-[10px] tracking-wide uppercase truncate">{slide.category}</span>
                            </div>
                            <p className="text-white text-sm font-medium truncate">{slide.title}</p>
                            <p className="text-amber-400 text-xs">{formatPrice(Number(slide.subtitle))}</p>
                          </div>
                          <button
                            aria-label={`View ${slide.title}`}
                            className="shrink-0 w-7 h-7 rounded-full bg-white/90 flex items-center justify-center"
                          >
                  ...
                  {/* Desktop: single full-width carousel */}
                  <div className="hidden md:block relative aspect-[4/3] sm:aspect-[16/10] lg:aspect-[16/9] min-h-[340px] sm:min-h-[420px] lg:min-h-[500px] w-full rounded-2xl sm:rounded-3xl overflow-hidden shadow-2xl border border-white/10 bg-[#161922] group touch-pan-y">
