...
export default function AdminLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // Support "admin" username shorthand -> admin@yourdomain.com per spec (reverted per request)
    if (email === 'admin') {
      email = 'admin@yourdomain.com';
    } else if (!email) {
      errs.email = 'Email is required';
    } else if (!email.includes('@')) {
      errs.email = 'Enter a valid email address (or "admin" for default account)';
    }
      ...
      <div className="w-full max-w-md rounded-xl border border-gray-200 bg-white p-8 shadow-sm">
        ...
        <form onSubmit={handleLogin} className="mt-8 space-y-4" noValidate>
          <div>
            ...
            <p className="mt-1 text-xs text-gray-500">Default: <span className="font-medium text-gray-700">admin / admin</span> maps to admin@yourdomain.com</p>
          </div>
        ...
        <p className="mt-6 text-center text-xs text-gray-500">Protected by RLS allowlist. Only <span className="font-medium text-gray-700">admin@yourdomain.com</span> can write.</p>
      </div>
    </div>
  );
}
